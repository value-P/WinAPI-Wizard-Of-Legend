...
int CFlame::Update()
{
	if (m_bDead || m_pOwner == nullptr) return OBJ_DEAD;
...
void CFlame::Late_Update()
{
	...
	{
		if (m_tFrame.Get_CurFrame() == 0 && !m_bOnShoot)
		{
			m_tFrame.Stop_Frame();
		}
		else if (m_tFrame.Get_CurFrame() == 0 && m_bOnShoot)
			m_tFrame.Resume_Frame();
		else if (m_tFrame.Get_CurFrame() == 4 && m_bOnShoot)
			m_tFrame.Stop_Frame();

	}
}

void CFlame::Render(HDC hDC)
{
	if (!m_bOnShoot) return;
...
void CFlame::Debug_Render(HDC hDC)
{
...
void CFlame::Anim_Update()
{
}

void CFlame::UpdateFrame(float _angle)
{
	float angle = 0.f;
...
SKILL_TYPE CFlame::Get_SkillType()
{
	return ST_THROW;
}
